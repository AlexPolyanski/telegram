...
def welcome_message(message):
    ...
    my_bot.send_message(message.chat.id, text='Ващ айди ' +
                        str(message.from_user.id))
    my_bot.send_message(
        message.chat.id, text='Выберете адрес:', reply_markup=markup)
...
def make_rec(message, **kwargs):
    ...
    ws.append([f'{current_indi.addr}, кв. {current_indi.appart}', f'{current_indi.hot}',
              f'{current_indi.cold}', f'{current_indi.elct}', time, user])
...
def confirm_addr(message, current_level, **kwargs):
    ...
    my_bot.send_message(chat_id=message.chat.id, text=text,
                        reply_markup=gen_markup(list_of_check, 3, current_level+1))
...
def appart_input(message, **kwargs):
    old_message = my_bot.edit_message_text(
        chat_id=message.chat.id, message_id=message.id, text='Введите номер квартиры:')
    current_level = 1
...
def user_text_input(message, current_level, old_message):
    my_bot.delete_message(chat_id=old_message.chat.id,
